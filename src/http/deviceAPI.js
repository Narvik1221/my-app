...
export const fetchFamiliesAutoSearch = async (id, userId) => {
  const { data } = await $host.get(
    "api/family/searchAuto/" + id + "/" + userId
  );
  return data;
};
export const fetchOneTree = async (id, currentTree, spouseId) => {
...
export const fetchSearchFIO = async (id, name,surname,patr) => {
  const { data } = await $host.get("api/family/fio/" + id, {
    params: {
        name:name,
        surname:surname,
        patr:patr
...
export const deleteSpouse = async (id, personId, familyId) => {
  const { data } = await $authHost.delete(
    "api/family/spouse/" + id + "/" + personId + "/" + familyId
  );
  return data;
};
export const deletePerson = async (id, familyId) => {
  const { data } = await $authHost.delete(
    "api/family/person/" + id + "/" + familyId
  );
  return data;
};
...
export const createBasketDevice = async (basketId, deviceId) => {
  const { data } = await $authHost.post("api/device/basket", {
    basketId,
    deviceId,
  });
  return data;
